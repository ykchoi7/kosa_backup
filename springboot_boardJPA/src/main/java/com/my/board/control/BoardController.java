...
public class BoardController {
	...
	public List<BoardDTO> list() throws FindException {
		return service.findAll();
	}
	
	@GetMapping("/{boardNo}") //1번 게시글 상세
	public BoardDTO info(@PathVariable int boardNo) throws FindException {
		return service.findByBoardNo(boardNo);
	}
	
	//POST 작성 /board 
	@PostMapping(value="", produces="application/json;charset=UTF-8") //게시글 쓰기
	public ResponseEntity<?> write(@RequestBody BoardDTO bdto) throws AddException {
		try {
			service.write(bdto);
...
//		return "글쓰기" + board;
	...
	public ResponseEntity<?> modify(@PathVariable int boardNo,
						 			@RequestBody BoardDTO bdto) throws ModifyException {
...
//		return "글삭제" + boardNo;
	...
	public ResponseEntity<?> writeReply(@PathVariable int boardNo,
			...
			@RequestBody ReplyDTO rdto) throws AddException { //쓰기에서 작성한 정보들을 한 번에 reply객체로 전달해줘야하기 때문에 @RequestBody를 사용한다
		...
		if(!optParentNo.isPresent()) { // 있는지, 없는지에 따라 true/false --> 일반 답글쓰기
			rdto.setReplyBoardNo(boardNo);
			service.writeReply(rdto);
		} else { //parentNo가 있는 경우 --> 답글의 답글쓰기
			Integer parentNo = optParentNo.get();
			rdto.setReplyParentNo(parentNo);
			rdto.setReplyBoardNo(boardNo);
			service.writeReply(rdto);
		}
		return new ResponseEntity<>(HttpStatus.OK);
	...
	public ResponseEntity<?> modifyReply(@PathVariable int replyNo,
										@RequestBody ReplyDTO rdto) throws ModifyException {
